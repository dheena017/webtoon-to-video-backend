...
(async () => {
  ...
  try {
    ...
    if (page.url().includes("/login") || page.url().includes("/landing")) {
      console.log(
        "Redirected to login/landing. Attempting to bypass or register..."
      );
      await page.goto("http://localhost:3000/register");
      await page.fill('input[placeholder*="Full Name"]', "Audio Tester");
      const email = `test_audio_${Math.floor(
      ...
      await page
        ...
        .catch(() => console.log("Navigation timeout after register"));
    }
    ...
    if (await settingsButton.isVisible()) {
      ...
      console.log(
        "Settings cog not found. Trying to click any button that might be it."
      );
