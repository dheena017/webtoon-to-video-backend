...
export function insertProject(data: Omit<Project, 'id' | 'created_at' | 'updated_at'>): number {
...
export function getProject(project_id: string): Project | undefined {
  return db.prepare('SELECT * FROM projects WHERE project_id = ?').get(project_id) as Project | undefined;
}

/** Update a project's status and panels_count. */
export function updateProject(project_id: string, updates: Partial<Pick<Project, 'status' | 'panels_count' | 'video_url'>>): void {
  db.prepare(`
    UPDATE projects
...
export function getPanels(project_id: string): Panel[] {
  return db.prepare('SELECT * FROM panels WHERE project_id = ? ORDER BY panel_index ASC').all(project_id) as Panel[];
}
...
export function saveEditHistory(editedUrl: string, originalUrl: string, editType = 'crop'): void {
  db.prepare(`
...
export function getEditHistory(editedUrl: string): EditHistoryEntry | undefined {
  return db.prepare('SELECT * FROM edit_history WHERE edited_url = ?').get(editedUrl) as EditHistoryEntry | undefined;
}

/** Get database statistics for the health check endpoint. */
export function getDbStats(): { projects: number; panels: number; sessions: number } {
  const projects = (db.prepare('SELECT COUNT(*) as c FROM projects').get() as any).c;
  const panels   = (db.prepare('SELECT COUNT(*) as c FROM panels').get() as any).c;
  const sessions = (db.prepare('SELECT COUNT(*) as c FROM scrape_sessions').get() as any).c;
