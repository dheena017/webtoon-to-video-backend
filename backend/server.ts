...
function rateLimitMiddleware(
  ...
  res.setHeader(
    ...
    String(Math.max(0, RATE_LIMIT_RPM - entry.count))
  );
  ...
  if (entry.count > RATE_LIMIT_RPM) {
    ...
    console.warn(
      `${col.warn("[RateLimit]")} ${col.error("BLOCKED")} IP ${c.yellow}${ip}${
        c.reset
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.setHeader(
    ...
    "X-Request-ID, X-Response-Time, X-API-Version, X-RateLimit-Remaining"
  );
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.on("finish", () => {
    ...
    const id = `${c.dim}[${((req as any).requestId || "").slice(0, 8)}]${
      c.reset
    }`;
    const slow = isSlow ? ` ${col.warn(`⚠ SLOW`)}` : "";
    const size =
      bodyLen > 0 ? ` ${c.dim}↑${(bodyLen / 1024).toFixed(1)}KB${c.reset}` : "";
    const resSz =
      resLen > 0 ? ` ${c.dim}↓${(resLen / 1024).toFixed(1)}KB${c.reset}` : "";
...
app.get("/api/metrics", (_req: Request, res: Response) => {
  ...
  res.json({
    ...
    memory: {
      ...
      systemUsedPct:
        (((os.totalmem() - os.freemem()) / os.totalmem()) * 100).toFixed(1) +
        "%",
    },
    config: {
      port: PORT,
...
function printStartupBanner(mode: string, port: number) {
  ...
  const cpuModel =
    os.cpus()[0]?.model.split(" ").slice(0, 4).join(" ") || "unknown";
...
${B}${M}║  ${
    c.brightYellow
  }🎬  ANIVOX WEBTOON-TO-VIDEO COMPILER ${M}— BACKEND SERVER v${API_VERSION}  ${M}║${R}
${B}${M}╠══════════════════════════════════════════════════════════════════╣${R}
${B}${M}║${R}  ${CY}Mode:${R}       ${W}${mode.padEnd(49)}${R}${B}${M}║${R}
${B}${M}║${R}  ${CY}Port:${R}       ${W}${String(port).padEnd(
    49
  )}${R}${B}${M}║${R}
...
${B}${M}║${R}  ${CY}Status:${R}     ${maint}${" ".repeat(
    Math.max(0, 49 - (MAINTENANCE_MODE ? 26 : 8))
  )}${B}${M}║${R}
${B}${M}╠══════════════════════════════════════════════════════════════════╣${R}
${B}${M}║${R}  ${B}${
    c.brightYellow
  }Route Groups:${R}                                                   ${B}${M}║${R}
...
${B}${M}║${R}  ${CY}NODE_ENV:${R}         ${W}${(
    process.env.NODE_ENV || "development"
  ).padEnd(43)}${R}${B}${M}║${R}
${B}${M}║${R}  ${CY}GEMINI_API_KEY:${R}   ${gemini}${" ".repeat(
    Math.max(0, 43 - (process.env.GEMINI_API_KEY ? 5 : 24))
  )}${B}${M}║${R}
${B}${M}║${R}  ${CY}Rate Limit:${R}       ${W}${(
    RATE_LIMIT_RPM + " req/min per IP"
  ).padEnd(43)}${R}${B}${M}║${R}
${B}${M}║${R}  ${CY}Slow Req Threshold:${R}${W}${(SLOW_REQ_MS + "ms").padEnd(
    43
  )}${R}${B}${M}║${R}
${B}${M}║${R}  ${CY}Request Timeout:${R}  ${W}${(REQ_TIMEOUT_MS + "ms").padEnd(
    43
  )}${R}${B}${M}║${R}
${B}${M}╚══════════════════════════════════════════════════════════════════╝${R}`);
}
  ...
  setInterval(() => {
    ...
    console.log(
      `${col.label("[Monitor]")} ` +
        ...
        `Errs: ${
          totalErrors > 0 ? col.error(String(totalErrors)) : col.muted("0")
        } | ` +
        `Slow: ${
          slowRequests > 0 ? col.warn(String(slowRequests)) : col.muted("0")
        } | ` +
        `Timeouts: ${
          timedOutReqs > 0 ? col.error(String(timedOutReqs)) : col.muted("0")
        }`
    );
    ...
    if (prunedIPs > 0) {
      console.log(
        `${col.muted(`[Monitor] Pruned ${prunedIPs} stale rate-limit buckets`)}`
      );
    }
  }, INTERVAL_MS);
}
...
async function startServer() {
  ...
  if (isProduction) {
    ...
    console.log(
      `${col.info("[Server]")} 🔄 Mounting ${col.value(
        "Vite"
      )} dev middleware...`
    );
  ...
  const shutdown = (signal: string) => {
    ...
    console.log(
      `${col.label("[Stats]")}  ` +
        ...
        `Data Sent: ${col.value(
          (totalBytesSent / 1024 / 1024).toFixed(1) + "MB"
        )}`
    );

    server.close(() => {
      console.log(
        `${col.success("[Server]")} ✅ HTTP server closed cleanly. ${
          c.dim
        }Goodbye!${c.reset}\n`
      );
      process.exit(0);
  ...
  process.on("uncaughtException", (err: Error) => {
    console.error(
      `${col.error("[Server]")} ❌ Uncaught Exception: ${col.error(
        err.message
      )}`
    );
  ...
  process.on("unhandledRejection", (reason: unknown) => {
    ...
    console.error(
      `${col.error("[Server]")} ❌ Unhandled Promise Rejection: ${col.error(
        msg
      )}`
    );
  });
