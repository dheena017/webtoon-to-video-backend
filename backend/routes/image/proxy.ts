  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (attempt < retries - 1) {
        ...
        console.warn(
          `${label("[Proxy]")} ${warn(
            `Network error, retry ${attempt + 1}/${retries}`
...
router.get("/proxy-image", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!["http:", "https:"].includes(parsed.protocol)) {
      return res
        .status(400)
        .json({ success: false, error: "Only HTTP/HTTPS URLs are supported." });
    }
  } catch {
    return res
      .status(400)
  ...
  try {
    ...
    if (contentLength > MAX_PROXY_SIZE) {
      console.warn(
        `${label("[Proxy]")} ${warn("Blocked oversized response")} ` +
          `${(contentLength / 1024 / 1024).toFixed(
            1
          )}MB > ${MAX_PROXY_SIZE_MB}MB limit`
      );
    ...
    if (buffer.length > MAX_PROXY_SIZE) {
      return res.status(413).json({
        success: false,
        error: `Downloaded image (${(buffer.length / 1024 / 1024).toFixed(
          1
        )}MB) exceeds limit`,
      });
    }
...
router.delete("/proxy-cache", (_req: Request, res: Response) => {
  ...
  proxyCache.clear();
  console.log(
    `${label("[Proxy]")} ${warn("Cache cleared")} — removed ${size} entries`
  );
