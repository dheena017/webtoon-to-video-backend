...
const DEFAULT_ANALYSIS = {
  ...
  motion_type: "zoom_in",
  visual_description:
    "A cropped illustration frame ready for cinematic playback.",
};
...
function validateAnalysis(raw: Record<string, unknown>) {
  return {
    ...
    sfx:
      typeof raw.sfx === "string" && raw.sfx.trim()
...
router.post("/analyze-image", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.warn(
      `${label("[Analyze]")} ${warn("Image fetch failed")} — ${
        fetchErr.message
  ...
  try {
    ...
    console.error(
      `${label("[Analyze]")} ${err("AI failed after retries")} ` +
        `model=${val(targetModel)} — ${aiErr.message} ${dim(
          `(${ms}ms)`
        )} — using fallback`
    );
    return res.json({
  ...
  try {
    ...
    console.warn(
      `${label("[Analyze]")} ${warn("Response parse failed")} — ${
        parseErr.message
      }`
    );
    analysis = DEFAULT_ANALYSIS;
  ...
  console.log(
    `${label("[Analyze]")} ${ok("✓")} model=${val(targetModel)} ` +
      ...
      `brightness=${
        brightness !== undefined ? val(String(brightness)) : dim("n/a")
      } ` +
      dim(`(${ms}ms)`)
  );
...
router.post("/analyze-batch", async (req: Request, res: Response) => {
  ...
  if (!Array.isArray(urls) || urls.length === 0) {
    return res
      ...
      .json({
        success: false,
        error: "Field 'urls' must be a non-empty array.",
      });
  }
  if (urls.length > 20) {
    return res
      .status(400)
      .json({ success: false, error: "Maximum 20 panels per batch request." });
  }

  const CONCURRENCY = 4;
  const results: {
    url: string;
    analysis: typeof DEFAULT_ANALYSIS;
    error?: string;
  }[] = [];
  const queue = [...urls];
  let completed = 0;
  ...
  console.log(
    `${label("[Batch Analyze]")} ${ok("Starting")} ` +
      `${val(String(urls.length))} panels @ concurrency=${val(
        String(CONCURRENCY)
      )} ` +
  ...
  async function processOne(url: string) {
    try {
      const resolved = await resolveImageToBuffer(url);
      const brightness = await computeBrightness(resolved.data).catch(
        () => undefined
      );
      ...
      console.log(
        `${label("[Batch]")} ${dim(`[${completed}/${urls.length}]`)} ` +
          `${ok("✓")} ${dim(url.slice(0, 50))} motion=${val(
  ...
  console.log(
    `${label("[Batch Analyze]")} ${ok("Complete")} ` +
      `${val(String(results.length))} panels ${dim(
        `(${ms}ms total, ~${Math.round(ms / results.length)}ms/panel)`
      )}`
  );
