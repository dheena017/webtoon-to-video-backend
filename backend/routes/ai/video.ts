...
router.post("/convert-images-to-video", async (req, res) => {
  ...
  if (!panels || !Array.isArray(panels) || panels.length === 0) {
    return res
      .status(400)
      .json({
        error: "A non-empty 'panels' array is required to compile a video.",
      });
  }
  ...
  try {
    ...
    for (let i = 0; i < panels.length; i++) {
      ...
      const audioArgs = [
        "--dialogue_list",
        JSON.stringify([panel.speech_text || ""]),
        "--target_duration",
        (panel.duration || 4.5).toString(),
        "--output_path",
        audioPath,
      ];

      console.log(
        `[Compile Video] Generating audio for panel ${i}: ${panel.speech_text?.substring(
          0,
          30
        )}...`
      );

      const { code: audioCode, stderr: audioStderr } = await runPythonScript(
        audioScript,
        audioArgs
      );
      if (audioCode !== 0) {
        throw new Error(
          `Audio generation failed for panel ${i}: ${audioStderr}`
        );
      }
    ...
    console.log(
      `[Compile Video] Spawning video compiler: ${videoScript} ${videoArgs.join(
        " "
      )}`
    );

    const { code: videoCode, stderr: videoStderr } = await runPythonScript(
      videoScript,
      videoArgs
    );
    if (videoCode !== 0) {
      console.error(
        `[Compile Video Error] Video compiler exited with code ${videoCode}: ${videoStderr}`
      );
    ...
    stitchedCache.set(videoCacheId, {
      data: videoBuffer,
    ...
    return res
      .status(500)
      .json({ error: `Video compilation failed: ${err.message || err}` });
  } finally {
    // Cleanup workspace
