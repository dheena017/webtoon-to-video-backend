...
router.post(
  ["/ai-detect-panels", "/detect-panels", "/ai-smart-crop"],
  async (req, res) => {
    const {
      url,
      model,
    ...
    try {
      ...
      if (strategy === "local-cv" || model === "local-cv") {
        ...
        const uniqueId = `detect_${Date.now()}_${Math.floor(
          Math.random() * 1000
        )}`;
        ...
        const args = [
          ...
          cannyHigh.toString(),
          "--close_kernel_size",
          closeKernelSize.toString(),
          "--min_height_px",
          minHeightPx.toString(),
        ];

        console.log(
          `[Local CV Detector API] Spawning ${pythonScript} with args: ${args.join(
            " "
          )}`
        );
        ...
        try {
          ...
          if (code !== 0) {
            console.error(
              "[Local CV Detector Error] Script failed:",
              code,
              stderr
            );
          ...
          if (!data.success) {
            throw new Error(
              data.error || "Local CV detection script reported failure."
            );
          }
          coordPanels = data.panels || [];
        } catch (cvErr: unknown) {
          console.error(
            "[Local CV Detector Error] Parse or execution error:",
            cvErr
          );
          throw cvErr;
        } finally {
          ...
          try {
            ...
            console.warn(
              "[Local CV Detector API Warning] Failed to clean up temp file:",
              unlinkErr
            );
          }
        }
        ...
        try {
          ...
          console.error(
            "[AI Smart Crop API] All retries failed or fatal error encountered. Returning empty.",
            err
          );
        ...
        console.log(
          `[AI Smart Crop] Gemini isolated ${coordPanels.length} panels.`
        );
      }
      ...
      return res
        .status(500)
        .json({ error: `AI Smart Crop failed: ${err.message || err}` });
    }
  }
);
