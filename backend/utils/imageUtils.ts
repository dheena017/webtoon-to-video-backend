...
export async function resolveImageToBuffer(
  ...
  if (
    ...
    const matched = workingUrl.match(
      /\/(?:merge|stitch)-images\/cached\/([^\\/\s?&]+)/
    );
    if (matched?.[1]) {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch image: ${response.status} ${response.statusText} — ${workingUrl}`
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (info.width >= 15 && info.height >= 15) trimmed = data;
        ...
        console.warn(
          "[ImageUtils] Trim produced near-empty image — skipping trim."
        );
    } catch {
      try {
        const { data, info } = await sharp(imageBuffer)
    ...
    let padding =
      typeof cropPadding === "number" ? cropPadding : tighter ? 4 : 20;
...
export async function stackVertical(
  ...
  if (buffers.length === 0)
    throw new Error("No buffers provided to stackVertical");
  if (buffers.length === 1)
    return { data: buffers[0], contentType: "image/jpeg" };
