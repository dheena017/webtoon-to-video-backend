...
export default function SoundOutroTab({
  ...
  const handleGenerate = async () => {
    if (!videoUrl) {
      if (addNotification) {
        addNotification("Please compile your video first on the Dashboard before generating soundtrack vibes & outro CTAs.", "warning");
      }
      return;
    }
    ...
    try {
      ...
      if (addNotification) {
        addNotification("Successfully compiled soundtrack recommendations and endings!", "success");
      }
    } catch (e) {
      console.error(e);
      if (addNotification) {
        addNotification("Failed to generate soundtrack vibes or end CTAs.", "error");
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="bg-amber-950/20 border border-amber-900/40 rounded-xl p-4 flex gap-3 text-amber-250 animate-fade-in">
            ...
            <div className="space-y-1 text-xs">
              ...
              <p className="text-neutral-400 leading-relaxed font-sans">
                Audio transitions and dynamic ending narrations cannot be computed accurately without the completed video duration. Please compile the video on the Dashboard first.
              </p>
            </div>
          </div>
        )}
