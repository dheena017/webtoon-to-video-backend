  ...
  const filteredLiveLogs = useMemo(() => {
    ...
    return consoleLogs.filter((log) => {
      const matchesSearch =
        ...
        (log.correlation_id &&
          log.correlation_id.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="bg-purple-950/20 border border-purple-500/20 rounded-3xl p-5 flex items-start gap-5 shadow-inner">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-purple-300/60 leading-relaxed font-mono">
            ...
            memory. Historical view queries the persistent SQL transaction log
            for deep forensic analysis. Logs are automatically pruned every 7
            days or when exceeding 5,000 records. Snapshot metadata is captured
