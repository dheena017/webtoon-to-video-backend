...
export function BubbleCleanerDetectionStyle({ detectionStyle, setDetectionStyle }: Props) {
  return (
    <div className="space-y-3">
      <SectionTitle icon={<Cpu className="h-3 w-3 text-purple-400" />}>What to Detect</SectionTitle>
      <div className="flex flex-col gap-2.5">
        {DETECTION_OPTIONS.map((opt) => (
          <button key={opt.value} onClick={() => setDetectionStyle(opt.value as DetectionStyle)}
