    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-[fadeIn_0.2s_ease-out]">
      ...
      <div className="lg:col-span-7 space-y-6">
        ...
        {showComparison && (
          <div className="p-4 bg-neutral-900/50 border border-indigo-500/30 rounded-2xl space-y-3 animate-fadeIn">
            <span className="text-[10px] font-mono font-bold text-indigo-400 uppercase">
              Slot Parameter Comparison
            </span>
            <div className="grid grid-cols-3 gap-2">
              {["slot1", "slot2", "slot3"].map((s) => (
                <div
                  key={s}
                  className="p-2 rounded-lg bg-neutral-950 border border-neutral-800 text-[8px] font-mono space-y-1"
                >
                  <div className="text-white font-bold border-b border-neutral-800 pb-1 mb-1 truncate">
                    {customPresets[s].name}
                  </div>
                  <div className="flex justify-between">
                    <span>SENS:</span>
                    <span>{customPresets[s].sensitivity}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>DIL:</span>
                    <span>{customPresets[s].bubbleDilation}px</span>
                  </div>
                  <div className="flex justify-between">
                    <span>RAD:</span>
                    <span>{customPresets[s].bubbleInpaintRadius}px</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <button
          ...
          className="w-full py-2 rounded-xl border border-neutral-800 text-[9px] font-bold text-neutral-500 hover:text-white transition-all uppercase tracking-widest"
        >
      ...
      <div className="lg:col-span-5 space-y-5">
        ...
        <BubbleCleanerPerfEstimator eraseMethod={props.eraseMethod} />
      </div>
    </div>
  );
}
