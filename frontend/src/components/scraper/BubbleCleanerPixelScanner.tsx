...
export function BubbleCleanerPixelScanner({
  firstImageUrl,
  sensitivity,
  setSensitivity,
  detectionStyle,
  ...
  const [histogramData, setHistogramData] = useState<
    { h: number; isTarget: boolean }[]
  >([]);
  const [maskPreviewUrl, setMaskColorPreviewUrl] = useState<string | null>(
    null
  );
  ...
  const autoSetSensitivity = () => {
    ...
    addNotification?.(
      `Auto-tuned sensitivity to ${clamped}% based on image histogram.`,
      "success"
    );
  };
  ...
  const triggerBubbleScan = () => {
    ...
    setTimeout(() => {
      setIsScanning(false);
      const targetWeight = histogramData.reduce(
        (acc, bin) => (bin.isTarget ? acc + bin.h : acc),
        0
      );
      const count = Math.round(targetWeight / 15);
      setScanResult(
        `Analysis complete: Isolated ${count} suspected speech bubble areas on active frame.`
      );
  ...
  return (
    <div className="bg-neutral-950/40 border border-neutral-800 rounded-3xl p-5 space-y-4">
      ...
      <div className="flex items-center justify-between text-[9px] font-mono">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={autoSetSensitivity}
            className="flex items-center gap-1 text-indigo-400 hover:text-indigo-300 transition-colors uppercase font-bold"
          >
          ...
          >
            {showHistogram ? "Hide Details" : "Show Details"}
          </button>
        </div>
      </div>
      ...
      <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden border border-neutral-800 bg-neutral-950 flex flex-col items-center justify-center">
        ...
        {maskPreviewUrl && (
          <div
            ...
            style={{
              backgroundImage: `url(${maskPreviewUrl})`,
              mixBlendMode: "screen",
            }}
          />
        )}
            ...
            {histogramData.map((bar, i) => (
              <div
                ...
                className={`flex-1 rounded-t-sm transition-all duration-500 ${
                  bar.isTarget
                    ? "bg-purple-400/80 shadow-[0_0_8px_rgba(168,85,247,0.4)]"
                    : "bg-neutral-700/30"
                }`}
                style={{ height: `${Math.max(2, bar.h)}%` }}
              />
            ))}
            <span className="absolute -top-3 left-2 text-[6px] font-mono text-neutral-400 uppercase tracking-tighter">
              Threshold Map (Level: {sensitivity})
            </span>
          </div>
        )}

        <span className="absolute top-2.5 right-2.5 text-[7px] px-2 py-0.5 rounded bg-black/80 border border-neutral-800 text-purple-300 font-mono tracking-wider z-10">
          Live Mask Overlay
        </span>
      </div>
      ...
      <div className="space-y-2.5 pt-1">
        ...
        >
          ...
          {isScanning
            ? "Processing Masks..."
            : "Run suspected Speech Bubble Scan"}
        </button>
        {scanResult && (
          <div className="p-3 rounded-xl bg-purple-950/10 border border-purple-900/35 text-[8.5px] font-mono text-purple-300 leading-normal animate-fadeIn">
            {scanResult}
          </div>
        )}
      </div>
