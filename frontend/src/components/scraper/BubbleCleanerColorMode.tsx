...
export function BubbleCleanerColorMode({
  maskColor,
  setMaskColor,
  firstImageUrl,
  tolerance,
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2 p-4 bg-neutral-950/40 border border-neutral-800 rounded-2xl">
          <div className="flex items-center justify-between">
            <span className="text-neutral-500 uppercase tracking-wider font-bold text-[9px] font-mono block">
              Target Bubble Color Mode
            </span>
            <button
          ...
          <div className="grid grid-cols-2 gap-2 text-[9px] font-mono">
            ...
            <button
              onClick={() => setTargetColorOption("custom")}
              className={`py-1.5 rounded-lg border text-center transition-all cursor-pointer ${
            ...
            <div className="flex items-center gap-3 pt-1.5 border-t border-neutral-900 animate-fadeIn">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="h-6 w-10 bg-transparent border-0 cursor-pointer rounded-md focus:outline-none"
                />
              ...
              <div
                className="ml-auto w-12 h-4 rounded border border-neutral-700 relative overflow-hidden"
              ...
              >
                ...
                />
                <div
                  className="absolute inset-0 bg-black/30 right-0 left-auto"
                  style={{ width: `${tolerance}%` }}
                />
              </div>
            </div>
        ...
        <div className="space-y-2 p-4 bg-neutral-950/40 border border-neutral-800 rounded-2xl">
          ...
          <div className="grid grid-cols-4 gap-1.5 text-[8px] font-mono text-center">
            {[
              {
                label: "💜 Purple",
                val: "rgba(168, 85, 247, 0.45)",
                active: "border-purple-500 text-purple-400 bg-purple-950/15",
              },
              {
                label: "❤️ Red",
                val: "rgba(239, 68, 68, 0.45)",
                active: "border-red-500 text-red-400 bg-red-950/15",
              },
              {
                label: "💚 Green",
                val: "rgba(16, 185, 129, 0.45)",
                active: "border-emerald-500 text-emerald-400 bg-emerald-950/15",
              },
              {
                label: "💙 Cyan",
                val: "rgba(6, 182, 212, 0.45)",
                active: "border-cyan-500 text-cyan-400 bg-cyan-950/15",
              },
            ].map(({ label, val, active }) => (
              <button
                key={val}
                onClick={() => setMaskColor(val)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {targetColorOption === "custom" && (
          <div className="p-4 bg-neutral-950/40 border border-neutral-800 rounded-2xl space-y-3 animate-fadeIn">
            <span className="text-neutral-500 uppercase tracking-wider font-bold text-[8px] font-mono block">
              Quick Palette Library
            </span>
            <div className="flex flex-wrap gap-2">
              {quickPalettes.map((p) => (
                <button
                  key={p.hex}
                  onClick={() => setCustomColorHex(p.hex)}
                  className={`flex items-center gap-2 px-2.5 py-1.5 rounded-xl border border-neutral-800 bg-neutral-900/50 hover:bg-neutral-800 transition-all ${
                    customColorHex === p.hex
        ...
        <div className="p-4 bg-neutral-950/40 border border-neutral-800 rounded-2xl space-y-3">
          ...
          <p className="text-[7.5px] text-neutral-600 leading-normal">
            Determines the allowed variance from the target color. Higher values
            expand the mask into nearby shades.
          </p>
        </div>
      </div>
    </div>
