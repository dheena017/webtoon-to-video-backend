      ...
      {showComparison && (
        <div className="p-4 bg-neutral-900/50 border border-indigo-500/30 rounded-2xl space-y-3 animate-fadeIn">
          <span className="text-[10px] font-mono font-bold text-indigo-400 uppercase">
            Preset Slot Comparison
          </span>
          <div className="grid grid-cols-3 gap-2">
            {["slot1", "slot2", "slot3"].map((s) => (
              <div
                key={s}
                className="p-2.5 rounded-lg bg-neutral-950 border border-neutral-800 text-[8px] font-mono space-y-1.5"
              >
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-indigo-400">
                    {customPresets[s].cropSensitivity}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>PAD:</span>
                  <span className="text-indigo-400">
                    {customPresets[s].cropPaddingPx}px
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>MERGE:</span>
                  <span className="text-indigo-400">
                    {customPresets[s].overlapMergeThreshold}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>STRAT:</span>
                  <span className="text-indigo-400">
                    {customPresets[s].useLocalCV ? "CV" : "AI"}
                  </span>
                </div>
              </div>
            ))}
