...
export function BubbleCleanerSensitivityControl({
  sensitivity,
  setSensitivity,
}: Props) {
  const card =
  ...
  return (
    <div className={card}>
      ...
      <p className="text-[8px] text-neutral-600 leading-normal">
        OpenCV adaptive contrast limit. Higher values sweep light colored bubble
      ...
      <div className="mt-2 text-[8px] font-mono text-emerald-500 bg-emerald-950/15 border border-emerald-950/30 px-2 py-1 rounded-lg">
        ...
        {sensitivity > 55
          ? "Light/Day pages"
          : "Dark/Night panel mode (Thresh optimized)"}
      </div>
    </div>
  );
}
