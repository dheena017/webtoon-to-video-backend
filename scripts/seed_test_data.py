...
def seed_data():
    ...
    try:
        ...
        cursor.executemany("""
            INSERT INTO series (
                id, user_id, title, author, cover_image, genre, synopsis
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """, series)
        ...
        cursor.executemany("""
            INSERT INTO chapters (
                id, series_id, episode_number, original_url, status, panels_count, video_url
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """, chapters)
        ...
        cursor.executemany("""
            INSERT INTO edit_history (edited_url, original_url, edit_type)
            VALUES (?, ?, ?)
        """, edit_history_entries)
        ...
        user_sessions = [
            ("sess_test111", "user_testuser1", "Chrome on Windows 11", "192.168.1.45", "New York, USA", 1),
            ("sess_test222", "user_testuser1", "Safari on Apple iPhone", "172.56.21.99", "San Francisco, USA", 1),
            ("sess_pro111", "user_creator_pro", "Firefox Developer Edition on Arch Linux", "84.22.100.12", "London, UK", 1),
            ("sess_pro222", "user_creator_pro", "Chrome on Apple macOS", "198.51.100.3", "Toronto, Canada", 0) # inactive
        ]
        cursor.executemany("""
        ...
        audit_logs = [
            ("user_testuser1", "User registration", "127.0.0.1", "Success"),
            ("user_testuser1", "Generated developer API key 'Local Dev Sandbox'", "192.168.1.45", "Success"),
            ("user_testuser1", "Failed login attempt (bad password)", "192.168.1.200", "Failed"),
            ("user_testuser1", "User login via Chrome on Windows 11", "192.168.1.45", "Success"),
            ("user_creator_pro", "Unlocked achievement badge 'badge_pro_creator'", "84.22.100.12", "Success"),
            ("user_creator_pro", "Redeemed reward points for +1000 compute credits", "84.22.100.12", "Success"),
            ("user_creator_pro", "Activated Multi-Factor Authentication (MFA)", "84.22.100.12", "Success")
        ]
        cursor.executemany("""
        ...
        invoices = [
            ("INV-2026-001-testuser", "user_testuser1", 19.00, "Paid"),
            ("INV-2026-002-testuser", "user_testuser1", 19.00, "Paid"),
            ("INV-2026-001-creatorpro", "user_creator_pro", 49.00, "Paid"),
            ("INV-2026-002-creatorpro", "user_creator_pro", 49.00, "Paid"),
            ("INV-2026-003-creatorpro", "user_creator_pro", 49.00, "Unpaid")
        ]
        cursor.executemany("""
            INSERT INTO user_invoices (invoice_id, user_id, amount, status)
            VALUES (?, ?, ?, ?)
        """, invoices)
        ...
        api_keys = [
            ("key_test123", "user_testuser1", "Local Dev Sandbox", "av_live_68fa...7e2a"),
            ("key_pro456", "user_creator_pro", "Production Deployment App", "av_live_21c8...a891"),
            ("key_pro789", "user_creator_pro", "CI/CD Deployment pipeline", "av_live_df84...f239")
        ]
        cursor.executemany("""
